...
switch ($action) {
    ...
    case 'get_user':
        $stmt = $conn->prepare("
            SELECT u.user_id, u.username, u.name, u.m_level, u.time, r.admin, r.user AS role_user
            FROM `user` u
    ...
    case 'get_audit_logs':
        ...
        $stmt = $conn->prepare("
            SELECT log_id, action, action_type, timestamp, ip_address, target_name, status, source
            FROM `audit_logs`
            WHERE user_id = ?
    ...
    case 'get_logs_by_type':
        ...
        $stmt = $conn->prepare("
            SELECT l.log_id, l.user_id, u.username, l.action, l.timestamp, l.ip_address, l.status, l.source
            FROM `audit_logs` l
    ...
    case 'get_logs_by_ip':
        ...
        $stmt = $conn->prepare("
            SELECT l.log_id, l.user_id, u.username, l.action, l.action_type, l.timestamp, l.status
            FROM `audit_logs` l
            LEFT JOIN `user` u ON l.user_id = u.user_id
            WHERE l.ip_address = ?
            ORDER BY l.timestamp DESC
    ...
    case 'create_notice':
        ...
        $stmt = $conn->prepare("
            INSERT INTO `notices` (message, type, is_active, created_at)
            VALUES (?, ?, 1, NOW())
        ");
    ...
    case 'set_user_role':
        ...
        $stmt = $conn->prepare("
            INSERT INTO `audit_logs`
                (user_id, action, action_type, ip_address, target_id, target_name, status, source)
            VALUES
                (?, 'Role changed by SecOps', 'UPDATE', 'secops-agent', ?, 'user', 'success', 'secops')
        ");
    ...
    case 'log_secops_action':
        ...
        $stmt = $conn->prepare("
            INSERT INTO `audit_logs`
                (user_id, action, action_type, ip_address, target_name, status, run_id, source)
            VALUES
                (?, ?, ?, ?, ?, 'success', ?, 'secops')
        ");
    ...
    case 'temporary_lock_user':
        ...
        $stmt = $conn->prepare("SELECT user_id, username FROM `user` WHERE username = ? LIMIT 1");
        ...
        $stmt = $conn->prepare("
            UPDATE `user`
            SET status = ?, locked_until = ?, lock_reason = ?
            WHERE username = ?
        ");
        ...
        $stmt = $conn->prepare("
            ...
            VALUES
                (?, 'User temporarily locked', 'SECURITY', 'secops-agent', ?, 'success', 'secops')
        ");
        $stmt->bind_param("is", $user['user_id'], $username);
        $stmt->execute();
